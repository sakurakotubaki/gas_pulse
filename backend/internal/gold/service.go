...
// Run updates and broadcasts the simulated gold price at the configured interval.
...
func (s *Service) UpdateNow() Tick {
	...
	change := (s.rng.Float64()*2 - 1) * 0.50
	if s.rng.Float64() < 0.08 {
		change += (s.rng.Float64()*2 - 1) * 2.00
	}
	next := math.Max(minPrice, math.Min(maxPrice, previous+change))
	status := "EQUAL"
	if next > previous {
		status = "UP"
